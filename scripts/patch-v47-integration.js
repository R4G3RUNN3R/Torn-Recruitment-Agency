...
app=replaceOnce(app,`    const renderer=renderers[state.page]||renderOverview;`,`    if(V47FactionPlatform._test.IMPLEMENTED_ROUTES.has(state.page)){\n      await V47FactionPlatform.renderPage(state.page,{persist:false});\n      rebuildNav();\n      bindHelp();\n      document.querySelector('.ra-shell')?.classList.remove('sidebar-open');\n      stopLogRefresh();\n      return;\n    }\n    const renderer=renderers[state.page]||renderOverview;`,'Faction route delegation');
