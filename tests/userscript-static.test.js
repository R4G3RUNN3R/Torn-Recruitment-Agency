...
const PINNED_RUNTIME='4de6d7f59f19765d3888b5a43284e3aca48ac719';
...
test('public userscript is the v4.7.0 modular bootstrap with immutable reviewed runtime requires',()=>{
  assert.match(boot,/@version\s+4\.7\.0/);
  ...
  assert.match(boot,/INSTALLER_VERSION\s*=\s*'4\.7\.0'/);
  assert.match(boot,/EXPECTED_APP_VERSION\s*=\s*'4\.7\.0'/);
  assert.match(boot,/app\.SCRIPT_VERSION/);
  assert.match(boot,/app\.start\(\)/);
});

test('v4.7.0 loads Company and Faction dependencies before the source application',()=>{
...
test('v4.7.0 preserves the direct and addEventListener click bridge',()=>{
...
test('v4.7.0 guards the shared DOM against duplicate worlds and legacy RA UI',()=>{
...
test('v4.7.0 keeps scrollable content, title-bar-only Settings and safe maximize restore',()=>{
