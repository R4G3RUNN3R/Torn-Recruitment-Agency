...
const PINNED_RUNTIME='76a95ba6e009dc16682cc8ef2ef689394f65edf8';
...
test('public userscript is the v4.8.2 active-search release with immutable v4.8.2 core requires',()=>{
  assert.match(boot,/@version\s+4\.8\.2/);
  ...
  assert.match(boot,/INSTALLER_VERSION\s*=\s*'4\.8\.2'/);
  assert.match(boot,/EXPECTED_APP_VERSION\s*=\s*'4\.8\.2'/);
  assert.match(boot,/app\.SCRIPT_VERSION/);
  assert.match(boot,/app\.start\(\)/);
});

test('v4.8.2 loads Company and Faction dependencies before the source application',()=>{
...
test('v4.8.2 preserves the direct and addEventListener click bridge',()=>{
...
test('v4.8.2 guards the shared DOM against duplicate worlds and legacy RA UI',()=>{
...
test('v4.8.2 keeps scrollable content, title-bar-only Settings and safe maximize restore',()=>{
...
test('v4.8.2 protects dark-theme table and Settings contrast from Torn host CSS',()=>{
