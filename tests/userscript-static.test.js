...
test('public userscript is the v4.8.0 simplified-shell release with immutable v4.7.6 core requires',()=>{
...
test('v4.8.0 loads Company and Faction dependencies before the source application',()=>{
...
test('v4.8.0 preserves the direct and addEventListener click bridge',()=>{
...
test('v4.8.0 guards the shared DOM against duplicate worlds and legacy RA UI',()=>{
...
test('v4.8.0 keeps scrollable content, title-bar-only Settings and safe maximize restore',()=>{
...
test('v4.8.0 protects dark-theme table and Settings contrast from Torn host CSS',()=>{
  assert.match(boot,/#ra-app \.ra-table tbody td\{color:var\(--ra-accent2\)!important\}/);
  ...
  assert.match(boot,/function syncPublicVersionLabel\(\)/);
});
...
test('v4.7 core keeps Smart Match local and messaging manual',()=>{assert.match(app,/Smart Match.*zero Torn API calls/i);assert.match(app,/private chat[\s\S]*press Send/i);assert.doesNotMatch(app,/autoSubmit\s*:\s*true/);assert.doesNotMatch(app,/pipelineStage\s*=\s*['"]Contacted['"]s*;.*message/s);});
