...
const PINNED_RUNTIME='a80b594603d414821b80a3a24587ed7002169686';
...
test('public userscript is the v4.8.1 active-search release with immutable v4.8.1 core requires',()=>{
  assert.match(boot,/@version\s+4\.8\.1/);
  ...
  assert.match(boot,/INSTALLER_VERSION\s*=\s*'4\.8\.1'/);
  assert.match(boot,/EXPECTED_APP_VERSION\s*=\s*'4\.8\.1'/);
  assert.match(boot,/app\.SCRIPT_VERSION/);
  assert.match(boot,/app\.start\(\)/);
});

test('v4.8.1 loads Company and Faction dependencies before the source application',()=>{
...
test('v4.8.1 preserves the direct and addEventListener click bridge',()=>{
...
test('v4.8.1 guards the shared DOM against duplicate worlds and legacy RA UI',()=>{
...
test('v4.8.1 keeps scrollable content, title-bar-only Settings and safe maximize restore',()=>{
...
test('v4.8.1 protects dark-theme table and Settings contrast from Torn host CSS',()=>{
