...
test('421px mobile shell keeps hamburger sidebar open and routes Company Discover by physical click',{timeout:60000},async()=>{
  ...
  try{
    ...
    assert.ok(after.x>=0,`Company Discover should be on-screen after hamburger: ${JSON.stringify(after)}`);
    assert.equal(after.clickable,true,`Company Discover must receive the physical click after hamburger: ${JSON.stringify(after)}`);
