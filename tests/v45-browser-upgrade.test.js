...
test('v4.4 persisted DB11 upgrades through additive DB15 via the public v4.7.2 bootstrap and remains physically routable in Chrome',{timeout:60000},async()=>{const server=await serve();const port=server.address().port;const browser=await puppeteer.launch({executablePath:chromePath(),headless:true,args:['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage']});try{const page=await browser.newPage();await page.setViewport({width:1440,height:900});await page.goto(`http://127.0.0.1:${port}/seed`);await page.waitForFunction(()=>window.__seedDone===true);assert.equal(await page.evaluate(()=>window.__seedError||''),'');await page.goto(`http://127.0.0.1:${port}/`,{waitUntil:'networkidle0'});await page.addScriptTag({content:BOOT});await page.waitForFunction(()=>document.getElementById('ra-app')&&['#ra-sidebar-launcher','#ra-launch'].some(s=>{const e=document.querySelector(s);return e&&getComputedStyle(e).display!=='none'}),{timeout:15000});assert.equal(await page.evaluate(()=>window.__startupError),'');
 const dbVersion=await page.evaluate(()=>RA_V45App.DB_VERSION);assert.equal(dbVersion,15);
 const db15Probe=await page.evaluate(async()=>{const companyConfig=await RA_V45App._test.companyRepositories.config.get();const vacancy=await RA_V45App._test.companyRepositories.vacancies.save({id:'db15-probe',name:'DB15 Probe',status:'Draft'});const listed=await RA_V45App._test.companyRepositories.vacancies.list();const factionConfig=await RA_V45App._test.factionRepositories.config.get();return{companyConfigKey:companyConfig.key,factionConfigKey:factionConfig.key,vacancyId:vacancy.vacancyId,listed:listed.some(v=>v.vacancyId==='db15-probe')};});assert.deepEqual(db15Probe,{companyConfigKey:'company',factionConfigKey:'faction',vacancyId:'db15-probe',listed:true});
 const launcher=await page.evaluate(()=>['#ra-sidebar-launcher','#ra-launch'].find(s=>{const e=document.querySelector(s);return e&&getComputedStyle(e).display!=='none'})||'');assert.ok(launcher);await physicalClick(page,launcher);await physicalClick(page,'[data-page="company-candidates"]');await page.waitForFunction(()=>document.getElementById('ra-page-title')?.textContent==='Company Candidates',{timeout:10000});assert.equal(await page.$eval('#ra-page-title',e=>e.textContent),'Company Candidates');assert.equal(await page.evaluate(()=>window.__startupError),'');
