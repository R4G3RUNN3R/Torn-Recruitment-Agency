...
test('v4.4 persisted DB11 upgrades through additive DB14 and remains physically interactive in Chrome',{timeout:60000},async()=>{const server=await serve();const port=server.address().port;const browser=await puppeteer.launch({executablePath:chromePath(),headless:true,args:['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage']});try{const page=await browser.newPage();await page.setViewport({width:1440,height:900});await page.goto(`http://127.0.0.1:${port}/seed`);await page.waitForFunction(()=>window.__seedDone===true);assert.equal(await page.evaluate(()=>window.__seedError||''),'');await page.goto(`http://127.0.0.1:${port}/`,{waitUntil:'networkidle0'});await page.waitForFunction(()=>window.__raStarted===true||window.__raStarted===false);assert.equal(await page.evaluate(()=>window.__startupError),'');assert.equal(await page.evaluate(()=>window.__raStarted),true);
 const dbVersion=await page.evaluate(()=>RA_V45App._test.state.db.version);assert.equal(dbVersion,14);
 const db14Stores=await page.evaluate(()=>[...RA_V45App._test.state.db.objectStoreNames]);for(const store of ['playerIntelligence','companyRecruitment','factionRecruitment','companyVacancies','companyCampaigns','companyRecruitmentConfig','companyRecruitmentSessions'])assert.ok(db14Stores.includes(store),store);
