...
Scout is the local player-intelligence subsystem inside Recruitment Agency v4.1. It is available as its own mode and also enriches Company/Faction recruitment records.
...
Recruitment Agency v4.1 uses IndexedDB database `tornWorkerDB`, version 9.

Stores:

- `users` - forum recruitment records from Company/Faction modes
- `meta` - application settings, synchronization history, UI mode, and window geometry
...
Recruitment Agency v4.1 uses one shared rate gate for Torn API traffic generated by the script.
