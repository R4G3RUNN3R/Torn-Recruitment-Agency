(function (root, factory) {
  ...
  function normalizeCompanyRecruitment(record = {}, observedAt = Date.now(), options = {}) {
    ...
    const base = {
      ...
      pinnedVacancyId:text(record.pinnedVacancyId),stageChangedAt:record.stageChangedAt ?? null,timelineEvents:Array.isArray(record.timelineEvents) ? record.timelineEvents.map(item => ({...item,payload:item?.payload?{...item.payload}:item?.payload})) : [],timelineNotes:Array.isArray(record.timelineNotes) ? record.timelineNotes.map(item => ({...item})) : [],
